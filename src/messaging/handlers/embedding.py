...
# Log Celery broker info on import
logger.info(f"[Handler] Celery broker: {celery.conf.broker_url}")
...
class EmbeddingRequestHandler:
    ...
    def _queue_add_user(self, message_id: str, client_slug: str, user_data: Dict[str, Any]):
        ...
        logger.info(f"[Handler] Submitting add_user task {task_id} to Celery (queue=embeddings)...")
        ...
        logger.info(f"[Handler] Celery returned: task_id={result.id}, state={result.state}")
    ...
    def _queue_update_user(self, message_id: str, client_slug: str, user_data: Dict[str, Any]):
        ...
        logger.info(f"[Handler] Submitting update_user task {task_id} to Celery (queue=embeddings)...")
    ...
    def _queue_delete_user(self, message_id: str, client_slug: str, user_data: Dict[str, Any]):
        ...
        logger.info(f"[Handler] Submitting delete_user task {task_id} to Celery (queue=embeddings)...")
