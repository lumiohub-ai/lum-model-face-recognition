...
class Settings:
    ...
    # Camera-worker routing (LSO-218). yolo routes each camera's `camera.track`
    # to its own `cam.<id>` queue; each camera-worker replica claims cameras
    # individually via a Redis lease (pipeline/camera_lease.py), up to its
    # capacity. No slot count lives here: the queue set is the live camera set,
    # and the fleet size is set by the camera-worker `replicas` + capacity env
    # (see src/camera_boot.py), not by a constant baked into this image.
