...
for cameras, tracking, detection, and phone usage logic.
"""
...
class PhoneDetectionConfig(BaseModel):
    """Configuration for phone detection using YOLOv8."""

    model_size: Literal["n", "s", "m", "l", "x"] = Field(
        default="n",
        description="YOLOv8 model size for phone detection"
    )
    confidence_threshold: float = Field(
        default=0.4,
        ge=0.0,
        le=1.0,
        description="Minimum confidence for phone detection"
    )
...
class StorageConfig(BaseModel):
    ...
    save_phone_usage_clips: bool = Field(
        default=True,
        description="Save video clips when phone usage detected"
    )
...
class CameraConfig(BaseModel):
    ...
    phone_detection: PhoneDetectionConfig = Field(
        default_factory=PhoneDetectionConfig
    )
    phone_usage: PhoneUsageConfig = Field(
        default_factory=PhoneUsageConfig
    )
