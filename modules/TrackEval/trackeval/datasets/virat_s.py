...
class ViratS(_BaseDataset):
    """Dataset class for VIRAT-S tracking"""
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            'GT_FOLDER': os.path.join(code_path, 'data/gt/virat_s/'),  # Location of GT data
            'TRACKERS_FOLDER': os.path.join(code_path, 'data/trackers/virat_s/'),  # Trackers location
            'OUTPUT_FOLDER': None,  # Where to save eval results (if None, same as TRACKERS_FOLDER)
            'TRACKERS_TO_EVAL': None,  # Filenames of trackers to eval (if None, all in folder)
            'CLASSES_TO_EVAL': ['person'],  # Valid: ['person']
            'BENCHMARK': 'virat_s',  # Valid: 'virat_s'
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        """Load a file (gt or tracker) in the VIRAT-S format
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        VIRAT-S:
            In VIRAT-S, the 4 preproc steps are as follow:
        ...
        distractor_class_names = ['static_person', 'distractor', 'reflection']
