...
class PersonPath22(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            # Location of GT data
            'GT_FOLDER': os.path.join(code_path, 'data/gt/person_path_22/'),
            # Trackers location
            'TRACKERS_FOLDER': os.path.join(code_path, 'data/trackers/person_path_22/'),
    ...
    def __init__(self, config=None):
        ...
        for tracker in tqdm(self.tracker_list):
            # print(tracker)
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        read_data, ignore_data = self._load_simple_text_file(
            file, is_zipped=self.data_is_zipped, zip_file=zip_file, crowd_ignore_filter=crowd_ignore_filter)
            ...
            if is_gt:
                if time_key in ignore_data.keys():
                    time_ignore = np.asarray(
                        ignore_data[time_key], dtype=float)
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            if self.do_preproc and self.benchmark != 'MOT15' and (gt_ids.shape[0] > 0 or len(crowd_ignore_regions) > 0) and tracker_ids.shape[0] > 0:
                ...
                to_remove_tracker = np.unique(np.concatenate(
                    [to_remove_tracker, np.where(is_within_crowd_ignore_region)[0]]))
